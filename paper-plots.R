...
for (dataset in c('gmp', 'eid')){
    ...
    round(effectiveSize(param_phy$w[c,burn]))
    ...
    round(effectiveSize(param_phy$eta[burn]))
    dev.off()
...
##############################################################################
    ...
    hist(log(P[com==0]),col=colnoass,main ='',
         ylab = 'Probability', freq=FALSE, xlab='Log of probability', ylim=c(0,0.4))
    hist(log(P[com>0]),col=colass,freq=FALSE, add=TRUE)
...
for(data in c('eid', 'gmp')){
    ...
    gres= lapply(nn.files, function(f){
        ...
        if(grepl('DistOnly', name)){
            if(!is.null(names(res[[1]]$param)))
                Eta = mean(sapply(res, function(r) r$param$eta)) else
            Eta = mean(sapply(res, function(r) r$param)) # to delete once we have new dist
    ...
    #gnames = sapply(gres, function(r) r[['name']])
...
colnames(xx)<-c('Model', 'AUC', 'Prediction', 'AUC', 'Prediction')
...
rm(list=ls())
files = grep('10foldCV-', list.dirs(recursive=FALSE), value=TRUE)
files = files[grepl('Uncertain', files)]
all.data = list()
rho_col <- "blue"
gamma_col <- "red"
eta_col <- "darkgreen"
...
for(f  in files){
    print(f)
    a =regexpr(paste0('[A-Za-z-0-9-]*foldCV-[A-za-z0-9-]*','(gmp|eid)'), f, perl=TRUE)
    name = substr(f, a, a + attributes(a)$match.length-1)
    
    
    #name = gsub('Uncertain-10foldCV-','' , name);name
    ...
    colass = rgb(0,0.8,0.8,0.5)
    colnoass = rgb(1,0,0.4,0.4,0.5)
    hist(log(P1[com10>0]),col=colass,main ='', ylab='Density', freq=FALSE, xlab='Log of probability',ylim=c(0, 0.38), xlim=c(-10,0))
    ...
    hist(paramG$g,freq=T,col=g_col, xlab='Posterior estimate of g for the GMP-Carnivora database', main='', breaks=40)
    ...
    ## adding affinities
    if(!is.null(res[[1]]$affinWithG)){
        aux = sapply(res, function(r) r[['affinWithG']]['param'])
        G = mean(sapply(aux, function(r) r[['g']]))
        W = rowMeans(sapply(aux, function(r) r[['w']]))
        Y = rowMeans(sapply(aux, function(r) r[['y']]))
        P = 1- exp(-outer(Y,W))
        PaffinG = G*P/(1-P + G*P)
        PaffinG[com>0]<-P[com>0]
        
        aux = sapply(res, function(r) r[['affinWithOutG']]['param'])
        W = rowMeans(sapply(aux, function(r) r[['w']]))
        Y = rowMeans(sapply(aux, function(r) r[['y']]))
        PaffinNoG = 1- exp(-outer(Y,W))

        rocAffinNoG = rocCurves(Z=1*(com10>0), Z_cross=com, P=PaffinNoG, all=TRUE, plot=FALSE, bins=400)
        rocAffinG= rocCurves(Z=1*(com10>0), Z_cross=com, P=PaffinG, all=TRUE, plot=FALSE, bins=400)
        
        cbind(Model=c('with G', 'without G'), rbind( round(ana.table(com10, com, rocAffinG), 4), round(ana.table(com10, com, rocAffinNoG), 4)))

        ## Full with G
        ord.pg = order(rocAffinG$P, decreasing=TRUE)
        Zpg = 1*(rocAffinG$P>rocAffinG$threshold)
        ## Full without G
        ord.p = order(rocAffinNoG$P, decreasing=TRUE)
        Zp = 1*(rocAffinNoG$P>rocAffinNoG$threshold)

        affinm = t(sapply(1:m, function(r) c(
            AffinWithG= sum(Z[ord.pg[1:r]]*Zpg[ord.pg[1:r]]),
            AffinWithOutG= sum(Z[ord.p[1:r]]*Zp[ord.p[1:r]])))) 
        topm = cbind(topm, affinm)

        tbg= data.frame(ana.table(com=1*(com10>0), comCross=com, roc = rocAffinG))
        tb= data.frame(ana.table(com=1*(com10>0), comCross=com, roc = rocAffinNoG))
        tb = round(rbind(tbg, tb),3)
        rownames(tb)=c('Affinity only with $g$', 'Affinity only without $g$')
        tb = data.frame(tb, MuG = mean(G))
        tb
        ...
        all.data[[paste0(name, '-affin')]]<-TBx

            wil[[paste0(name, '-affin')]]<-cbind(all = wilcoxon.local.test(unlist(sapply(res, function(r)
        r[['affinWithG']][['tb.all']]['auc'])),
                           unlist(sapply(res, function(r) r[['affinWithOutG']][['tb.all']]['auc']))),
                       subset = wilcoxon.local.test(unlist(sapply(res, function(r)
                           r[['affinWithG']][['tb']]['auc'])),
                           unlist(sapply(res, function(r) r[['affinWithOutG']][['tb']]['auc']))))
    ...
    ## adding DistOnly
    if(!is.null(res[[1]]$DistOnlyWithG)){
        aux = sapply(res, function(r) r[['DistOnlyWithG']]['param'])
        G = mean(sapply(aux, function(r) r[['g']]))
        Eta = mean(sapply(aux, function(r) r[['eta']]))
        P = 1- exp(-((phy_dist^Eta)%*%com) )
        PG = G*P/(1-P + G*P)
        PG[com>0]<-P[com>0]
        
        aux = sapply(res, function(r) r[['DistOnlyWithOutG']]['param'])
        Eta = mean(sapply(aux, function(r) r[['eta']]))
        PNoG = 1- exp(-((phy_dist^Eta)%*%com) )

        rocDistOnlyNoG = rocCurves(Z=1*(com10>0), Z_cross=com, P=PNoG, all=TRUE, plot=FALSE, bins=400)
        rocDistOnlyG= rocCurves(Z=1*(com10>0), Z_cross=com, P=PG, all=TRUE, plot=FALSE, bins=400)
        
        cbind(Model=c('with G', 'without G'), rbind( round(ana.table(com10, com, rocDistOnlyG), 4), round(ana.table(com10, com, rocDistOnlyNoG), 4)))

        ## Full with G
        ord.pg = order(rocDistOnlyG$P, decreasing=TRUE)
        Zpg = 1*(rocDistOnlyG$P>rocDistOnlyG$threshold)
        ## Full without G
        ord.p = order(rocDistOnlyNoG$P, decreasing=TRUE)
        Zp = 1*(rocDistOnlyNoG$P>rocDistOnlyNoG$threshold)

        Distm = t(sapply(1:m, function(r) c(
            DistOnlyWithG= sum(Z[ord.pg[1:r]]*Zpg[ord.pg[1:r]]),
            DistOnlyWithOutG= sum(Z[ord.p[1:r]]*Zp[ord.p[1:r]])))) 
        topm = cbind(topm, Distm)

        tbg= data.frame(ana.table(com=1*(com10>0), comCross=com, roc = rocDistOnlyG))
        tb= data.frame(ana.table(com=1*(com10>0), comCross=com, roc = rocDistOnlyNoG))
        tb = round(rbind(tbg, tb),3)
        rownames(tb)=c('Phylogeny only with $g$', 'Phylogeny only without $g$')
        tb = data.frame(tb, MuG = mean(G))
        tb
        ...
        all.data[[paste0(name, '-dist')]]<-TBx

            wil[[paste0(name, '-dist')]]<-cbind(all = wilcoxon.local.test(unlist(sapply(res, function(r)
        r[['DistOnlyWithG']][['tb.all']]['auc'])),
                           unlist(sapply(res, function(r) r[['DistOnlyWithOutG']][['tb.all']]['auc']))),
                       subset = wilcoxon.local.test(unlist(sapply(res, function(r)
                           r[['DistOnlyWithG']][['tb']]['auc'])),
                           unlist(sapply(res, function(r) r[['DistOnlyWithOutG']][['tb']]['auc']))))
    ...
    if(any(grepl('Affin', colnames(topm)))){
        lines(x=1:m,y = topm[,'AffinWithG'], lty=1, type='l', lwd=2, col='darkgreen')
        lines(x=1:m,y = topm[,'AffinWithOutG'], lty=3, type='l', lwd=2, col='darkgreen')
        gnames = c(gnames, 'Affinity-only with uncertainty', 'Affinity-only')
        gcol = c(gcol, 'darkgreen', 'darkgreen')
        glwd = c(glwd, 2,2)
        glty = c(glty, 1,3)
    }

    
    if (any(grepl('Dist', colnames(topm)))){
        lines(x=1:m,y = topm[,'DistOnlyWithG'], lty=1, type='l', lwd=2, col='brown')
        lines(x=1:m,y = topm[,'DistOnlyWithOutG'], lty=3, type='l', lwd=2, col='brown')
        gnames = c(gnames, 'Phylogeny-only with uncertainty', 'Phylogeny-only')
        gcol = c(gcol, 'brown', 'brown')
        glwd = c(glwd, 2,2)
        glty = c(glty, 1,3)
    }
...
eid.only = grepl('(model|eid)', colnames(AUCtb), ignore.case=TRUE)
print(xtable(AUCtb[,eid.only], digits=3),
      include.colnames = TRUE,
      include.rownames = FALSE,
      sanitize.text.function=function(x){x},
      only.contents=TRUE,file='tb-EID-AUC.tex')
...
eid.only = grepl('(model|eid)', colnames(PRED), ignore.case=TRUE)
...
print(xtable(PRED[,eid.only]),
      include.colnames = TRUE,
      include.rownames = FALSE,
      sanitize.text.function=function(x){x},
      only.contents=TRUE, file='tb-EID-Pred.tex')

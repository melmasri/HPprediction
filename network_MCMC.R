...
network_est<-function(Z, slices = 10, tree = NULL, model.type = c('full', 'distance', 'affinity'), uncertainty = FALSE,sparse=TRUE, ... ){
    ...
    if(grepl('(dist|full)', model.type)){
        ...
        param  = ICM_est_over_acc(unname(Z),tree,slices, distOnly = grepl('dist', model.type),  uncertainty = uncertainty, sparse=sparse, ...)
    }
...
### ==================================================
rEta.over.acc<-function(eta.old,tree,tree.ht,pdist.old, pdist0,pdist00, sZ, Z, ywU,
                        eta_sd =0.01, a, b,nr, ind, sparse, mr){
    ## a faster version of rEta using faster cophenetic and sparse matrices
    eta.prop = eta.old + eta_sd*rnorm(1)
    dist = cophFast(eb.phylo(tree, tree.ht, eta.prop), a, b,nr)
    pdist.new = dist%*%sZ
    ...
    ##acc
    acc = sum(pmin(1, exp(acc))*mr/sum(mr))
...
ICM_est_over_acc<-function(Z, tree, slices = 10, distOnly = FALSE, uncertainty = FALSE, sparse=TRUE, ...){
    ## Main function for ICM, only applied is when distance input is used
    ## loading extra args
    el <-list(...)
    ## parameters set-up
    ...
    tryCatch(
        for(s in 1:slices){
            ...
            ## Updating similarity matix parameter
            new.eta = rEta.over.acc(peta[s],tree,tree.ht,pdist,
                pdist0,pdist00, sparseZ, Z,outer(y0[,s+1], w0[,s+1])*U0,
                eta_sd, lowerIndex, upperIndex,ny, ind, sparse, mr)

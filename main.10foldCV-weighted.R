...
res = mclapply(1:tot.gr ,function(x, pairs, Z, dist, hyper){
    ...
    param = gibbs_one(com_paCross,slice=slice ,dist= dist, eta=1, hyper=hyper,updateHyper=FALSE, AdaptiveMC = TRUE)
...
},pairs=pairs,Z = com, dist=phy_dist, hyper=hyper, mc.preschedule = TRUE, mc.cores = max(5, tot.gr) )
